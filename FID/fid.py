...
def get_activations(images, sess, batch_size=40, verbose=False):
...
def calculate_activation_statistics(images, sess, batch_size=40, verbose=False):
...
def get_activations_from_files(files, sess, batch_size=40, verbose=False):
    """Calculates the activations of the pool_3 layer for all images.
...
def calculate_activation_statistics_from_files(files, sess, batch_size=40, verbose=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def calculate_fid_given_paths(paths, inception_path, low_profile=True):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        np.savez('test.npz', mu=m1, sigma=s1)
        print("path1 images is done.")
        m2, s2 = _handle_path(paths[1], sess, low_profile=low_profile)
        print("path2 images is done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lowprofile", default=True, action="store_true",
    ...
    save_path = args.path[1].split("valid")[0] + "fid.txt"
    print(save_path)
    print("FID: ", fid_value)
    f = open(save_path, 'w')
    f.writelines("FID: %.6f\n" % (fid_value))
