...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code):
        ...
        # text_temp = text_temp.contiguous().view(-1, self.gf_dim, 4, 4)
        # out_code = out_code + text_temp
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ngf = self.gf_dim
        self.att = SPATIAL_NET(ngf, self.ef_dim)   # 空间注意力
        self.channel_att = CHANNEL_NET(ngf, self.ef_dim)   # 通道级注意力
        self.residual = self._make_layer(ResBlock, ngf * 3)
        self.upsample = upBlock(ngf * 3, ngf)

    def forward(self, h_code, c_code, word_embs, mask):
        """
            h_code(query):  batch x idf x ih x iw (queryL=ihxiw)  # hidden state
        ...
        h_c_code = torch.cat((h_code, c_code), 1)   # 隐状态和空间注意力特征 连接
        h_c_c_code = torch.cat((h_c_code, c_code_channel), 1)  # 之后再和通道级注意力特征 连接
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            ...
            # if att_c1 is not None:
            #     att_c_maps.append(att_c1)
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3, att2 = \
            ...
            # if att_c2 is not None:
            #     att_c_maps.append(att_c2)
