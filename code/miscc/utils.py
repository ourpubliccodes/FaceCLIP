    ...
    for i in range(num):
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
                ...
                one_map *= 255
                # one_map.transpose(1,2,0)
                #
                # print(img.shape, type(img))   # (128,128,3)
                # print(one_map.shape, type(one_map))  # (128,128,6)
                PIL_im = Image.fromarray(img.astype('uint8'))
                PIL_att = Image.fromarray(one_map.astype('uint8'))
...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
                        attn_maps, att_sze, vis_size=256, topK=18):
