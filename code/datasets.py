...
class TextDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.split == 'train' or (self.is_single and self.split == 'test' or 'valid'):
        ...
        # if (self.split == 'test' or 'valid') and (not self.is_single):
        #     # randomly select 5 sentences
        #     sent_ix = random.sample(range(0, self.embeddings_num), 5)  # 是个长度为5的列表
        #     new_sent_ix = [i + (index * self.embeddings_num) for i in sent_ix]
        #     temp_caps, temp_cap_len = [], []
        #     for item in new_sent_ix:
        #         caps, cap_len = self.get_caption(item)
        #         temp = caps.tolist()
        #         temp_caps.append(temp)
        #         temp_cap_len.append(cap_len)
        #     caps = np.array(temp_caps)
        #     cap_len = temp_cap_len
        #
        #     # randomly select 5 mismatch sentences
        #     wrong_idx = random.randint(0, len(self.filenames))
        #     wrong_new_sent_ix = [i + (wrong_idx * self.embeddings_num) for i in sent_ix]
        #     temp_wrong_caps, temp_wrong_cap_len = [], []
        #     for item in wrong_new_sent_ix:
        #         wrong_caps, wrong_cap_len = self.get_caption(item)
        #         temp = wrong_caps.tolist()
        #         temp_wrong_caps.append(temp)
        #         temp_wrong_cap_len.append(wrong_cap_len)
        #     wrong_caps = np.array(temp_wrong_caps)
        #     wrong_cap_len = temp_wrong_cap_len
        #     wrong_cls_id = self.class_id[wrong_idx]
