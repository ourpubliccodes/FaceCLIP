...
class SpatialAttention(nn.Module):
    ...
    def forward(self, input, context):  # input是上一个阶段的隐状态，contex是词嵌入
        """
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, weightedContext, context, ih, iw):
        ...
        ## channel attention
        batch_size, sourceL = context.size(0), context.size(2)
        sourceC = context.unsqueeze(3)  # 表示在第3维加一个维度，即升维
        ...
        attn_c = torch.bmm(weightedContext, sourceC)  # 计算两个tensor的矩阵乘法
